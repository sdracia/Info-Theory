...
def create_run_folder():
    # Trova l'ultimo numero di run esistente
    i = 1
    while os.path.exists(f'run_{i}'):
        i += 1
    
    # Crea la nuova cartella con il numero successivo
    run_folder = f'run_{i}'
    os.makedirs(run_folder)  # Crea la cartella
...
def run_simulation(N, K, T, seed, num_rep, pi, gamma, r, type_run):
    ...
    for i in range(num_rep):
        ...
        pi_est, gamma_est, r_est, running_nmi, running_perc_corr_class, delta_gamma, delta_r, delta_pi = em_algorithm(y, z, pi, gamma, r, N, K, max_iter=15)
